...
from level4_data_mgt import app, db
...
env = Environment(loader=PackageLoader('level4_data_mgt', 'json_templates'),
...
class Project(db.Document):
    ...
    created_date = db.DateTimeField(required=True)
    data_file_md5_lst = db.ListField(
                            db.StringField(max_length=32, required=True),
                            required=True
                        )
    data_file_name_lst = db.ListField(
    ...
    src_project_id_ref_lst = db.ListField(
                                db.ReferenceField('Project', required=True),
                                required=True
                             )
    src_project_name_lst = db.ListField(
...
def get_project_info(project_name, **kwargs):
    ...
    try:
        ...
        if project:
            return (False,
                    'success',
                    json.loads(render_template(project_info_j2_file,
                                               project=project,
                                               **kwargs)))
...
def get_all_projects(**kwargs):
    ...
    try:
        for project in Project.objects:
            app.logger.info(project)
            if project:
                project_info = json.loads(render_template(project_info_j2_file,
                                                          project=project,
                                                          **kwargs))
...
def exist_subproject(project_name, subproject_name):
    if Project.objects(project_name=project_name,
                       src_project_name_lst__exists=subproject_name):
        return True
    else:
        return False
...
def get_mutation_by_project(project_name, subproject_name,
                            gene_ensembl_id=None, transcript_ensembl_id=None,
                            **kwargs):
    query_args = {}
    ...
    try:
        ...
        if mutation_lst:
            ...
            results = [json.loads(render_template(mutation_j2_file,
                                                  mutation_info=mutation_info,
                                                  **kwargs))
                       for mutation_info in mutation_lst]
            return (False, 'success',
                    uniq_item(merge_dicts(merge_expr_obj, *results)))
        else:
...
def exist_ensembl_id(project_name, subproject_name, ensembl_id, expr_cls):
    ...
    projects = Project.objects(project_name=project_name)
    if expr_cls.objects(**kargs, project__in=projects,
                        subproject_name=subproject_name):
        return True
    else:
        app.logger.warning("未找到%s, %s, %s, %s" %
                           (project_name, subproject_name, ensembl_id))
        return False


def get_expr_by_project(project_name, subproject_name, ensembl_id, expr_cls,
                        **kwargs):
    query_args = {}
    if expr_cls.__name__ == 'TranscriptExpr':
    ...
    try:
        ...
        expr_value_rest_lst = expr_cls.objects(**query_args,
                                               project_ref__in=projects)
        ...
        if expr_value_rest_lst:
            ...
            results = [json.loads(render_template(expr_value_j2_file,
...
def get_expr_info_by_lst(project_name, subproject_name, id_lst, expr_cls,
                         **args):
    '''
    返回表达数据，多个ID同时查询
    '''
    ...
    for transcript_id in id_lst:
        error, message, expr_info = get_expr_by_project(project_name,
                                                        subproject_name,
                                                        transcript_id,
                                                        expr_cls, **args)
...
class ExprInfo(db.Document):
    ...
    clinical_data_id_lst = db.ListField(db.ReferenceField('ClinicalData',
                                                          required=True))
    expr_value_lst = db.ListField(db.FloatField(), required=True)
    transcript_ensembl_id_lst = db.ListField(db.StringField(max_length=50,
                                                            required=True))
    gene_ensembl_id_lst = db.ListField(db.StringField(max_length=50,
                                                      required=True))
    project_ref = db.ReferenceField(Project, required=True)
    subproject_name_lst = db.ListField(db.StringField(max_length=30,
                                                      required=True))
    project_name = db.StringField(max_length=30, required=True)
    samples_data_id_lst = db.ListField(db.ReferenceField('SampleData',
                                                         required=True))
...
def get_exprtemp_info(id=None, query_condition_md5=None, **kwargs):
    ...
    try:
        if query_condition_md5:
            expr_info = ExprInfo.objects.get(
